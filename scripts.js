 ...
 ----------------
   <script type="text/javascript">
    ...
    <script>
     jQuery('#discountVal').val(localStorage.getItem("discountCode"));
    </script>  
  ...
  Shopify.updateCartInfo = function(cart, cart_summary_id, cart_count_id) {
		if ((typeof cart_summary_id) === 'string') { 
         ...
         if( disCountValue != '' ){  
          jQuery('#product-info .discount-codes div').each(function(){ 
            if( jQuery(this).attr('coupon-title') == disCountValue ){
              pDiscount = jQuery(this).attr('data-discount'); 
            }
          });
         }   
					...
					if (key === 'items') {
						...
						if (value.length) {
							...
							jQuery.each(value, function(i, item) { 
                              ...
                              if(pDiscount != ''){
                                discountPrice = item.price - ((parseFloat(pDiscount)/100.00) * item.price);
                              } else {
                                            ...
                                            <div class="row"> \
                                                ...
                                                <div class="col-md-8 col-sm-8 col-xs-8">\
                                                    <div class="cart-info">\
                                                        ...
                                                        <div class="product-price">' + Shopify.formatMoney(discountPrice, "{{shop.money_format}}")  + '</div>\
                                                    </div>\
                                                </div>\
                                            </div>\
											<a class="remove-link" title="{{ 'general.header.cart_info.remove' | t }}" href="javascript:;" data-cart-remove="' +  (i + 1) + '"></a>\
                                ...
                                totalPrice = parseFloat(totalPrice) + ( parseFloat(discountPrice) * item.quantity );                                                                          
							});
							                           
                            jQuery('<div class="cart-summary text-center inner-top-50"> \
								<h5 class="cart-total"> {{ 'general.header.cart_info.subtotal' | t }}</h5> \
                                <p class="cart-total-price">' + Shopify.formatMoney(totalPrice, "{{shop.money_format}}")  + '</p>\
								<p class="instruction">Free Priority Shipping On All Orders!</p> \
								<a href="/cart\" class="btn btn-default btn-uppercase">{{ 'general.header.cart_info.view_shopping_bag' | t }}</a> \
                                <button class="btn btn-primary btn-uppercase continue-shopping" onclick="window.location=\'/checkout/?discount='+disCountValue+'\'">{{ 'general.header.cart_info.checkout' | t }}</button> \
							</div>').appendTo(table);
